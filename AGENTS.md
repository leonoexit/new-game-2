...
- Dùng [bản đồ nguồn luật](docs/current/README.md) để tìm contract sở hữu từng cơ chế và [danh mục lá V0 JSON](docs/current/LITTLE-VALLEY-CARD-REGISTER-V0.json) cho identity, loại, nơi và vai trò; sửa qua Card Register devtool hoặc sửa JSON rồi chạy `python3 scripts/run_prototype.py --check`. Không tự suy nguồn chuẩn từ tên thư mục hay tên file.
