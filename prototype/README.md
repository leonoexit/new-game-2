...
Tìm, lọc, sắp xếp và chọn lá để đọc đủ 15 trường. Năm trường đầu khớp lá full: `Identity`, `Type`, `State`, `Action`, `Description`; trường phát triển nằm bên dưới. Nhấn **Add card** để tạo lá, **Edit entry** để sửa, hoặc **Add state** để thêm state cho identity đang chọn. State dùng nhãn `Base identity (State)`, ví dụ `Soil (Tilled Soil)`; devtool tự gợi ý nhãn này. Ba trường `Identity`, `Type`, `Base identity` là bắt buộc. `Type` chọn từ từ vựng chuẩn trong JSON, không nhập hậu tố `card` hay chú giải vào tên loại. Có thể gán ảnh runtime sẵn trong `prototype/assets/`; không cần điền mọi trường phát triển ngay. “No runtime image assigned” chỉ nghĩa là chưa gán art.
