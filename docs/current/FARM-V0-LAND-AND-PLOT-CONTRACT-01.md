...
- **Sức chứa đất Field V0 là hữu hạn, tối đa 8 vị trí đất/luống cùng lúc.** Cả tám vị trí đã hiện diện từ đầu. Dọn bụi đổi **ảnh/state trong cùng lá Soil**; đập đá **thay lá đá bằng lá Soil đất trống ở chính vị trí cũ**. `Till` dùng Hoe để đổi **state/ảnh của cùng lá Soil** từ trống sang đã cày; `Plant` đổi lá trên cùng vị trí sang card identity Crop theo giống; `Harvest` trả vị trí về lá `Soil` ở state đã cày. Không thêm vị trí thứ chín bằng mốc ngày, nhiệm vụ, mua đất hoặc cuộn giao diện trong V0. Đây là trần **vị trí đất**, không phải trần mọi loại lá con Field.
- Mỗi ô đất Field chứa tối đa **một** luống/crop theo CG-25. **CG-36:** Farmhouse, Shipping Bin và Building/Equipment khác là lá ở tableau Home, không tiêu suất đất Field. `TV` nằm trong Farmhouse; `Supermarket` không thuộc Home/Field.
...
`Empty Soil`, `Overgrown Soil` (bụi) và `Tilled Soil` là **tên trạng thái để đọc/lọc của cùng identity `Soil`**; đá vẫn là lá riêng. `Soil`, `Field Rock` và từng Crop có mục Bách Khoa riêng khi từng identity hiện; đổi ảnh bụi → đất trống → đã cày trong cùng lá Soil không mở mục mới. CG-40 sửa phân loại `Tilled Soil` identity ở CG-02/34; quyết định Plant đổi identity Soil → Crop theo giống vẫn giữ. Không lấy hình học của ô Field để quy đổi thành phạm vi tưới nhiều lá; CG-26 vẫn giữ Watering Can thường tưới một crop/1 AP trong V0.
...
**Chưa chốt trong phạm vi cơ chế Field:** vị trí/thứ tự cụ thể của 3 bụi và 3 đá; chi tiết UI lọc/tóm tắt lá đất. **Để đến giai đoạn đổ nội dung:** công dụng, giá bán và nguồn Stone khác. `Clear 1 AP` đã chốt theo CG-39; `Till` dùng Hoe, 1 AP, đổi state trong lá Soil theo CG-40. Không suy ra `Wood`, `Axe` hoặc phần thưởng từ bụi rậm.
