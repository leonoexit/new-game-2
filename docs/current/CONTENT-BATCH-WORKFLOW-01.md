...
5. **Đưa vào chơi:** bản nhập `imported_draft` ở `docs/content-batches/` là **kho ứng viên**, chưa là lá V0. Sau lượt rà, chỉ phần được chọn và có đường action/output/state đủ để chơi mới vào [decision log](ON-TABLE-CARD-SHAPE-DECISION-09.md), contract sở hữu và [danh mục V0](LITTLE-VALLEY-CARD-REGISTER-V0.xlsx) **theo cả lô**. Batch chọn một phần dùng `partially_selected` và `promotion` liệt kê chính xác biến thể/output đã vào V0; phần còn lại vẫn là nháp. Lỗi giá, công dụng phụ hoặc balance không chặn bước này; lỗi khiến action không thể giải quyết thì xử lý ở cấp họ trước khi chơi. Không đồng bộ snapshot lịch sử.
